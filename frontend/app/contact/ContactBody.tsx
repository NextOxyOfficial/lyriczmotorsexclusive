...
export default function ContactBody({ settings }: ContactBodyProps) {
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      value: phone,
      href: whatsapp ? `https://wa.me/${whatsapp.replace(/\D/g, '')}` : `tel:${phone.replace(/\s/g, '')}`,
  ...
  ] as { icon: typeof MapPin; label: string; value: string; href?: string; sub: string }[]
  ...
  const socials = [
    ...
    settings.youtube_url && { label: 'YouTube', href: settings.youtube_url, icon: Youtube },
  ].filter(Boolean) as { label: string; href: string; icon: typeof Facebook }[]
      ...
      <section className="relative overflow-hidden border-b border-white/10 py-5 sm:py-8">
        ...
        <div className="relative mx-auto max-w-7xl px-1 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-8">
            ...
            <div className="hidden sm:flex flex-col items-end gap-3 sm:max-w-xs lg:max-w-sm">
              <p className="text-sm leading-6 text-slate-400 text-right">
                Questions about a bike, spare part, or service? Drop us a message — our team responds fast.
              </p>
              <Link
                href="/#book"
                className="inline-flex items-center gap-2 bg-ignition px-4 py-2.5 text-xs font-black uppercase text-white clip-panel hover:bg-ignition/90 transition"
              >
                <CalendarClock className="h-3.5 w-3.5" /> Book a Service Slot
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_2fr]">
          ...
          <div className="flex flex-col gap-3">
            {contactInfo.map(({ icon: Icon, label, value, href, sub }) => (
              <div key={label} className="border border-white/10 bg-white/[0.04] p-3 sm:p-4 clip-panel">
            ...
            {/* Socials */}
            {socials.length > 0 && (
              <div className="border border-white/10 bg-white/[0.04] p-3 sm:p-4 clip-panel">
                <p className="mb-2.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Follow Us</p>
            ...
            {/* Book slot — mobile only */}
            <Link
              href="/#book"
              className="flex sm:hidden items-center justify-center gap-2 border border-ignition/40 bg-ignition/10 px-4 py-3 text-xs font-black uppercase text-ignition clip-panel hover:bg-ignition/20 transition"
            >
              <CalendarClock className="h-3.5 w-3.5" /> Book a Service Slot
            </Link>
          </div>
          ...
          {/* ── Right: Form ── */}
          <div className="border border-white/10 bg-white/[0.04] p-4 clip-panel sm:p-6">
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
                <div>
                  ...
                  <h2 className="text-lg font-black uppercase text-white sm:text-xl">Send a Message</h2>
                </div>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-volt px-6 py-3.5 text-sm font-black uppercase text-asphalt clip-panel hover:bg-volt/90 active:scale-95 transition disabled:opacity-60"
                >
